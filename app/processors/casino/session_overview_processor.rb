...
class CASino::SessionOverviewProcessor < CASino::Processor
  ...
  def process(cookies = nil, user_agent = nil)
    ...
    if tgt.nil?
      ...
      ticket_granting_tickets = tgt.user.ticket_granting_tickets.where({awaiting_two_factor_authentication: false, awaiting_acceptto_authentication: false}).order('updated_at DESC')
